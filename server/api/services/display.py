...
def _query_grid_samples(session: Session, plant_id: str, start: datetime, end: datetime) -> dict[datetime, Sample]:
    rows = session.execute(
        select(RawTelemetry.time, RawTelemetry.grid_power_kw)
        .where(
            RawTelemetry.plant_id == plant_id,
            RawTelemetry.time >= start,
            RawTelemetry.time <= end,
        )
...
def _query_battery_samples(
    ...
    column = RawTelemetry.battery_power_kw if field_name == "battery_power_kw" else RawTelemetry.soc
    rows = session.execute(
        select(RawTelemetry.time, column)
        .where(
            RawTelemetry.plant_id == plant_id,
            RawTelemetry.time >= start,
            RawTelemetry.time <= end,
