...
usage() {
  cat <<'EOF'
用法:
  server/scripts/docker_release.sh --image <镜像仓库地址> [--version <版本号>] [--platform <平台>] [--base-image <Python基础镜像>] [--node-base-image <Node基础镜像>] [--apt-mirror <Debian源>] [--apt-security-mirror <Debian安全源>] [--pip-index-url <PyPI源>] [--push]

示例:
  server/scripts/docker_release.sh \
...
说明:
  ...
  --node-base-image Node 基础镜像；默认 node:20-bookworm-slim。Docker Hub 网络不好时可切换镜像源。
...
NODE_BASE_IMAGE="${MPC_ONLINE_NODE_BASE_IMAGE:-node:20-bookworm-slim}"
...
服务器部署时使用:
  ...
  docker compose -f server/deploy/docker-compose.release.yml pull
  docker compose -f server/deploy/docker-compose.release.yml up -d
EOF
