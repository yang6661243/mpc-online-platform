...
usage() {
  cat <<'EOF'
用法:
  server/scripts/ecs_bootstrap_mpc_online.sh [选项]
...
常用示例:
  ...
  MPC_ONLINE_IMAGE=registry.cn-guangzhou.aliyuncs.com/your_namespace/mpc-online:20260612-001 \
    bash server/scripts/ecs_bootstrap_mpc_online.sh --install-docker

  # 方式二：服务器从 GitHub 拉源码后本地构建，适合镜像仓库还没准备好时测试。
  bash server/scripts/ecs_bootstrap_mpc_online.sh --install-docker --build-local
...
start_service() {
  ...
  if [[ "${BUILD_LOCAL}" -eq 1 || -z "${IMAGE}" ]]; then
    ...
    compose -f server/deploy/docker-compose.online.yml up -d --build
  else
    echo "拉取镜像并启动 MPC 服务: ${IMAGE}"
    export MPC_ONLINE_IMAGE="${IMAGE}"
    export MPC_ONLINE_PORT="${PORT}"
    compose -f server/deploy/docker-compose.release.yml pull
    compose -f server/deploy/docker-compose.release.yml up -d
  fi
}

verify_service() {
  echo "检查容器状态..."
  compose -f server/deploy/docker-compose.release.yml ps || compose -f server/deploy/docker-compose.online.yml ps || true
