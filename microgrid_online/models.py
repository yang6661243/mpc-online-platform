...
class RawGridMeter(Base):
    __tablename__ = "raw_grid_meter"
    __table_args__ = (UniqueConstraint("plant_id", "time", name="uq_raw_grid_meter_plant_time"),)
...
class RawBattery(Base):
    __tablename__ = "raw_battery"
    __table_args__ = (UniqueConstraint("plant_id", "time", name="uq_raw_battery_plant_time"),)

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    plant_id: Mapped[str] = mapped_column(String(64), index=True)
    time: Mapped[datetime] = mapped_column(DateTime(timezone=True), index=True)
    battery_power_kw: Mapped[float] = mapped_column(Float)
    soc: Mapped[float] = mapped_column(Float)
    battery_available: Mapped[bool] = mapped_column(Boolean, default=True)
    pcs_available: Mapped[bool] = mapped_column(Boolean, default=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now)
